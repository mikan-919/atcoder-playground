...
コンテストを丸ごと開くと、全問題のテンプレート作成とサンプル取得をまとめて行い、TUIで一覧を監視します。

```bash
bun run contest abc329
```

問題ごとに列が並び、各サンプルの合否が縦に表示されます。`solutions/abc329/<問題>/main.ts`か`src/`を保存すると、その問題だけ自動で再テストされます。

```
abc329  2/7 AC   a-z: 選択  r: 全再実行  q: 終了
...
`a`〜`g`のキーで問題を選ぶと、下部に最初に落ちたサンプルの入力・期待・実際の出力が表示されます。提出は`bun run submit -- abc329/b`のように別ターミナルから行います。
...
| コマンド | 内容 |
| --- | --- |
| `bun run contest abc329` | コンテスト全問を用意し、TUIで並列にサンプル監視 |
